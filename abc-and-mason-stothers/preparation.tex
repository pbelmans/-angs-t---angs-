...
\iftex
Some parts of this little overview use the alternative phrasing of the~$abc$\nobreakdash-conjecture as stated in Remark~\ref{remark:abc-with-factor}.
\fi
\ifblog
I repeat a statement from the previous post, for sake of completeness.
\begin{remark}
  \label{remark:abc-with-factor}
  We could also phrase the~$abc$\nobreakdash-conjecture in terms of a constant~$K_{\epsilon}$. In that case we have for~$\epsilon>0$ the existence of a~$K_{\epsilon}>1$ such that
  \begin{equation}
    c\leq K_{\epsilon}\rad(abc)^{1+\epsilon}.
  \end{equation}
  Hence the constant~$K_{\epsilon}$ swallows all finitely many counterexamples of the inequality~$c>\rad(abc)^{1+\epsilon}$.
\end{remark}
\fi
