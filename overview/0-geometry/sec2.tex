...
\end{equation}
and we know already that~$\sum_{P \in C} \operatorname{ord}_P(f)=0$. The \emph{genus\emph}~$\genus_C$ of the curve~$C$ is then determined from that number by~$2\genus_C - 2 = \sum_{P \in C}\operatorname{ord}_P(\omega)$.
...
\begin{example}
  ...
  \end{equation}
  Thus,~$\sum_{P \in \mathbb{P}^1} \operatorname{ord}_P(\omega) = -2$ and so the genus of the projective line~$\genus_{\mathbb{P}^1} = 0$.
\end{example}
